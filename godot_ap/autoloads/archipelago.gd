...
func load_packed_console_as_scene(tree: SceneTree, console: PackedScene) -> bool:
	...
	if not AP_Util.for_all_nodes(console.instantiate(), func(node): return node is ConsoleContainer):
		return false
...
func load_console(console_scene: Node, as_child := true) -> bool:
	...
	if console_scene is ConsoleContainer:
		...
		output_console_container = AP_Util.for_all_nodes(console_scene,
			func(node):
