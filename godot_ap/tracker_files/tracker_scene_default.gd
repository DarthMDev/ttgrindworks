...
func sort_click(event: InputEventMouseButton, column_name: String) -> bool:
	...
	if event.button_index == MOUSE_BUTTON_LEFT:
		...
		btn_checkall.pressed.connect(func():
			AP_Util.for_all_nodes(vbox, func(node: Node):
				if node is CheckBox:
		...
		match column_name:
			COL_HINT_STAT:
				var arr: Array = []
				arr.append_array(AP_Util.reversed(NetworkHint.status_names.keys()))
				for s in arr:
			...
			COL_LOC_STAT:
				var arr: Array = []
				arr.append_array(AP_Util.reversed(TrackerManager.statuses))
				for s in arr:
