...
class ConsoleDrawData:
	...
	func newline(c: BaseConsole, count := 1):
		var at_start := AP_Util.approx_eq(x, l)
		x = l
		if count > 0:
			if at_start:
				reset_y = y
	...
	func ensure_spacing(c: BaseConsole, spacing: Vector2):
		...
		x = l + spacing.x
		if not AP_Util.approx_eq(r_y, t):
	...
	func _calc_and_draw(c: BaseConsole, data: ConsoleDrawData, do_draw: bool) -> void:
		...
		while true:
			...
			if trim_pos <= text_pos: # No space at all, window is too thin
				if AP_Util.approx_eq(data.x, data.l):
					if space_only:
						space_only = false
...
class SpacingPart extends ConsolePart: ## A part that adds spacing
	...
	func draw(c: BaseConsole, data: ConsoleDrawData) -> void:
		...
		if from_reset_y:
			...
			data.x += spacing.x
			if not AP_Util.approx_eq(data.y, data.t):
				data.y += spacing.y
	...
	func textpart_replace(from_str: String, to_str: String, case_sens := true,
		...
		while q < parts.size():
			...
			if p is TextPart:
				...
				if repl:
					parts.pop_at(q)
					for p2 in AP_Util.reversed(repl):
						parts.insert(q, p2)
					q += repl.size()
					continue
			q += 1
...
func _get_mouse_pos() -> Vector2:
	return get_viewport().get_mouse_position() - global_position + AP_Util.MOUSE_OFFSET
...
func _draw() -> void:
	...
	if scroll > max_scroll:
		...
		queue_redraw.call_deferred()
	if AP_Util.approx_eq(scroll,_draw_data.max_scroll()):
		is_max_scroll = true
	if scroll_bar:
		scroll_bar.max_value = max_scroll
		scroll_bar.value = scroll
		scroll_bar.visible = max_scroll > AP_Util.GAMMA
...
func scroll_by_abs(amount: float) -> void:
	...
	if not AP_Util.approx_eq(scroll,old_scroll):
		update_scroll()
func _update_scroll2(_junk) -> void:
	return update_scroll()
func update_scroll() -> void:
	is_max_scroll = AP_Util.approx_eq(scroll,_draw_data.max_scroll())
	queue_redraw()
func _gui_input(event):
	if Engine.is_editor_hint(): return
	if event is InputEventMouseButton:
		for part in AP_Util.reversed(parts):
			if part.try_click(event, _get_mouse_pos()):
				return
		var fac: float = 1.0 if event.factor < AP_Util.GAMMA else event.factor
