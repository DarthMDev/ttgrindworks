...
func bk_wb() -> Tween:
	...
	append_cog_speak_shot(scene, bk, 'We may be able to harness this "core" to create a perfect foremen.', 4.0, false)
	append_cog_speak_shot(scene, wb, "Harness it?! Users exhibit behavior against what Cog Nation stands for.", 4.0,)
	...
	append_cog_speak_shot(scene, wb, "How would the chairman react to such workplace tomfoolery", 4.0, false)
	append_cog_speak_shot(scene, bk, "The chairman does like results.", 3.2,)
	#scene.tween_callback(wb.set_animation.bind('buffed'))
	append_cog_speak_shot(scene, bk, "It's true that we have seen some unperfered sideffects to exposure.", 4.0, false)
	append_cog_speak_shot(scene, bk, "But we lack sufficient data.", 3.0)
	append_cog_speak_shot(scene, bk, "It may be possible to minimize the drawbacks while retaining the increased power.", 4.0)
	append_cog_speak_shot(scene, wb, "Fine we have enough foremen to spare. It wouldn't be taxxing to allocate some for... controlled experiments.", 8.0)
	append_cog_speak_shot(scene, bk, "Excellent we should start-.", 1.75)
	#sg.show()
