...
const SG := preload('res://objects/cog/presets/lawbot/scapegoat.tres')
...
func bk_wb() -> Tween:
	...
	append_char_move(scene, player, get_char_position('WalkInPos'), 3.0, true)
	scene.parallel().set_trans(Tween.TRANS_QUAD).tween_property(camera, 'global_transform', get_camera_angle('FocusBoss'), 3.0)
	scene.parallel().tween_callback(wb.speak.bind("Foremen levitiating and spinning on the job...")).set_delay(2.0)
	scene.tween_interval(3.0)
	
	append_cog_speak_shot(scene, wb, "This behaviour is outrageous!.")
	append_cog_speak_shot(scene, bk, "Right.Their behaviour is peculiar indeed.")
	append_cog_speak_shot(scene, bk, "But those foremen are undoubtly stronger.", 3.0, false)
	append_cog_speak_shot(scene, bk, 'We may be able to harness this "core" to create a perfect foremen.', 5.5, false)
	append_cog_speak_shot(scene, wb, "Harness it?! Users exhibit behavior against what Cog Nation stands for.", 6.0,)
	append_cog_speak_shot(scene, wb, "Dare I say, part of them resemble acts of a toon", 4.0)
	#scene.tween_callback(wb.set_animation.bind('speak'))
	append_cog_speak_shot(scene, wb, "How would the chairman react to such workplace tomfoolery", 5.0, false)
	append_cog_speak_shot(scene, bk, "The chairman appreciates results.", 3.2)
	#scene.tween_callback(wb.set_animation.bind('buffed'))
	append_cog_speak_shot(scene, bk, "It's true that we have seen some unperfered sideffects to exposure.", 6.0, false)
	append_cog_speak_shot(scene, bk, "But we lack sufficient data.", 3.0)
	append_cog_speak_shot(scene, bk, "It may be possible to minimize the drawbacks while retaining the increased power.", 8.0)
	append_cog_speak_shot(scene, wb, "Fine we have enough foremen to spare. It wouldn't be taxxing to allocate some for... controlled experiments.", 11.0)
	append_cog_speak_shot(scene, bk, "Excellent we should start-.", 2.5)
	#sg.show()
	scene.tween_callback(func(): sg.visible = true)
	scene.tween_callback(sg.set_animation.bind('slip-forward'))
	scene.tween_callback(sg.animator.set_speed_scale.bind(0.05))
	scene.tween_callback(battle_node.focus_character.bind(sg))
	scene.tween_interval(5.0)
	#scene.set_trans(Tween.TRANS_LINEAR)
	append_char_turn(scene, wb, get_char_position('WalkInPos'), 1.5, false)
	append_cog_speak_shot(scene, wb, "What in the...", 1.7, false)
	append_char_turn(scene, bk, get_char_position('WalkInPos'), 1.5, true)
	append_cog_speak_shot(scene, wb, "Slacking on the job WHILE in front of a toon", 3.7, true)
	append_cog_speak_shot(scene, sg, "...", 3, true)
	append_cog_speak_shot(scene, wb, "Your infraction will be dealt immediately after we CRUSH this toon", 3.7, true)
	#scene.set_trans(Tween.TRANS_LINEAR)
	#append_char_turn(scene, wb, get_char_position('WalkInPos'), 1.5, true)
	scene.parallel().tween_callback(start_music)
	append_cog_speak_shot(scene, bk, "While the goat is doing... whatever that is.", 4.0, true)
	append_cog_speak_shot(scene, bk, "The infractions by you... Toon are truly monumental.", 4.5, false)
	append_cog_speak_shot(scene, bk, "You will be punished. Exetensively.")
	scene.tween_callback(sg.animator.set_speed_scale.bind(1))
	scene.tween_interval(0.75)
	scene.tween_callback(battle_node.reposition_cogs)
	scene.set_trans(Tween.TRANS_QUAD).tween_property(camera, 'global_transform', get_camera_angle('BattleFocus'), 2.0)
	scene.tween_interval(1.0)
	append_cog_speak_shot(scene, wb, "Let’s book ‘em, shall we?")
	append_cog_speak_shot(scene, bk, "I’m afraid that your story ends here.") #"You won't like me maa-d!"
	append_cog_speak_shot(scene, sg, "You won't like me maa-d!") 
